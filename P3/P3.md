...
### **g) Comparación Espectral de los Métodos de Filtrado**

```matlab
Nfft = 4096; % FFT grande para mayor resolución

% FFT de cada señal (calculadas sobre Nfft puntos)
% Usamos las variables definidas anteriormente: x, y_manual, g_conv_adj, h_filter
X = fftshift(fft(x, Nfft)) / length(x);
Y = fftshift(fft(y_manual, Nfft)) / length(x);
G = fftshift(fft(g_conv_adj, Nfft)) / length(x);
H = fftshift(fft(h_filter, Nfft)) / length(x);

% Eje de frecuencias centrado en 0
f_axis_g = (-Nfft/2 : Nfft/2-1) * (fs / Nfft);

% Representación gráfica
figure('Name', 'Comparación Espectral de Métodos');
plot(f_axis_g, abs(X), 'k', 'LineWidth', 1.5); hold on;
plot(f_axis_g, abs(Y), 'r--', 'LineWidth', 1.2);
plot(f_axis_g, abs(G), 'b:', 'LineWidth', 1.2);
plot(f_axis_g, abs(H), 'g-.', 'LineWidth', 1.2);

grid on;
xlim([-fs/2 fs/2]); % Mostramos todo el rango de Nyquist
legend('X[n] original', 'y[n] manual', 'g[n] conv', 'h[n] filter');
xlabel('Frecuencia (Hz)');
ylabel('Magnitud normalizada');
title('Comparación Espectral Centrada (Muestreo original)');

% Zoom opcional en la banda de paso si el filtro es muy estrecho
% xlim([-20000 20000]); % Descomenta esta línea si quieres ver mejor el detalle
```

![1773183542859](image/P3/1773183542859.png)
