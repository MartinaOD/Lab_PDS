...
```
H(z)=G⋅∏k=1M(z−pk)∏i=1M(z−zi)
```

donde zi son los ceros del filtro (raíces del numerador****Num_e).

**Cuando el filtro se evalúa en la circunferencia unidad (**z=ejω), si un cero zi se encuentra exactamente sobre dicha circunferencia (∣zi∣=1), la distancia entre el punto de evaluación y el cero se hace cero. Esto provoca que el numerador de la expresión sea igual a cero y, por tanto: ∣H(e^jω)∣ = 0      (−∞dB).
