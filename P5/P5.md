...
% Frecuencia normalizada (0 – 0.5)
f_norm = f / Fs;

% Representación
figure;
plot(f_norm, 20*log10(abs(H_sos)), 'b','LineWidth',1.2);
hold on;
plot(f_norm, 20*log10(abs(H_sos_q)), 'r--','LineWidth',1.2);
grid on;

title('Respuesta en frecuencia - Secciones de Segundo Orden');
xlabel('Frecuencia normalizada (f / Fs)');
ylabel('Ganancia (dB)');
